...
export class Meetup {
    ...
    renderLocationLink(venue) {
        if (venue) {
            if (venue.hasOwnProperty('lat') && venue.hasOwnProperty('lon')) {
                return (<a href={`https://www.google.com/maps/search/?api=1&query=${venue.lat},${venue.lon}`} target="_blank"><span>{venue.address_1},</span> {venue.city}</a>);
            } else {
    ...
    injectScript() {
        ...
        const url    = `https://api.meetup.com/2/events.json?&sign=true&callback=meetupCallback&photo-host=public&page=20&group_id=${this.getGroupIds()}`;
